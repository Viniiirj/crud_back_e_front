...
function renderizarTabela(pessoas) {
    ...
    pessoas.forEach((pessoa) => {
        ...
        <tr>
            ...
            <td>${pessoa.nome}</td>
            <td>${pessoa.email}</td>
            <td>${pessoa.dataNasc ? pessoa.dataNasc.split('-').reverse().join('/') : ''}</td>
            <td>
                <button class="btn-editar" onclick="editarPessoa(${pessoa.id})">Editar</button>
                <button class="btn-deletar" onclick="deletarPessoa(${pessoa.id})">Deletar</button>
            </td>
        </tr>
    `;
    });
